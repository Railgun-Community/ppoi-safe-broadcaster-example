...
class RestApi {
  ...
  async startRestApi () {
    try {
      ...
      // P2WDB commands
      this.router.post('relayer/', this.relayerApiHandler)
  ...
  // This function handles incoming REST API calls for wallet functions.
  async relayerApiHandler (ctx, next) {
    try {
      // console.log('Ping from apiHandler()')
      ...
      const { params } = rpcData

      // Generate a JSON RPC command.
      const cmd = _this.jsonrpc.request(rpcId, method, params)
      const cmdStr = JSON.stringify(cmd)
      console.log('cmdStr: ', cmdStr)

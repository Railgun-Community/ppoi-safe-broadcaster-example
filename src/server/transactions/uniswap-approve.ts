...
export async function populateUniswapApprovalTransactions(
  ...
  try {
    ...
    const permit2Contract = new Contract(permit2Address, ABI_PERMIT2, provider);
    ...
    if (typeof universalAllowance !== 'undefined') {
      if (universalAllowance.amount < tokenAmount.amount) {
        ...
        // eslint-disable-next-line no-await-in-loop
        const populatedPermit2Transaction =
          await permit2Contract.approve.populateTransaction(
            tokenAmount.tokenAddress,
            universalRouter,
            tokenAmount.amount,
            unit48timeout,
